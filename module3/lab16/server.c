...
void receive_file(int sock, const char *filename) {
    ...
    while ((bytes_recv = recv(sock, buffer, sizeof(buffer), 0)) > 0) {
        fwrite(buffer, 1, bytes_recv, file);
    }

    fclose(file);
    printf("Файл %s получен и сохранен как %s\n", filename, new_filename);
}
...
void dostuff(int sock) {
    ...
    while (1) {
        // Получение данных от клиента (команда или операция)
        ...
        // Если не "exit" или "file", считаем, что это операция
        op = buffer[0];
